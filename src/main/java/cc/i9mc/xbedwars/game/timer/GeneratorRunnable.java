...
import cc.i9mc.xbedwars.game.event.Runnable;
import cc.i9mc.xbedwars.utils.ArmorStandUtil;
...
public class GeneratorRunnable {
    ...
    public void start() {
        if (!this.timer) {
            ...
            //D
            game.getEventManager().registerRunnable("钻石时间显示", (seconds, currentEvent) -> Bukkit.getScheduler().runTask(XBedwars.getInstance(), () -> {
            ...
            game.getEventManager().registerRunnable("钻石刷新", (seconds, currentEvent) -> Bukkit.getScheduler().runTask(XBedwars.getInstance(), () -> game.getMapData().getDropLocations(MapData.DropType.DIAMOND).forEach(location -> location.getWorld().dropItem(location, new ItemBuilderUtil().setType(Material.DIAMOND).setDisplayName("§a§a§a§a§a§a").getItem()).setVelocity(new Vector(0.0D, 0.1D, 0.0D)))), 30);

            //E
            game.getEventManager().registerRunnable("绿宝石时间显示", (seconds, currentEvent) -> Bukkit.getScheduler().runTask(XBedwars.getInstance(), () -> {
